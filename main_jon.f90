...
integer, allocatable                 :: neighbor_list(:,:,:,:), N_neighbor(:,:), &
                                        neighbor_order_list(:,:,:), &
                                        cont_pdf(:,:,:), cont_adf(:,:,:), &
                                        mat_neighbor(:,:), mat_neighbor_in(:,:), mat_pairs(:,:)
...
logical :: compute_first_neighbor, plot_results, compute_deviations, check_constraints, read_mat_neighbor
integer :: ext, e1
...
read(1,*) 
read(1,*) read_mat_neighbor

allocate(mat_neighbor_in(N_species, N_species))
if (read_mat_neighbor) then
    do e1 = 1, N_species
        read(1,*) mat_neighbor_in(e1,:)
    enddo
endif
...
if (read_mat_neighbor) mat_neighbor = mat_neighbor_in
